...
namespace CSM.Logic
{
    public class CreateComponentPresenter : IPresentor
    {
        ...
        protected void MakeComponents(DAL parentDB, int numComponents)
        {
            ...
            List<ComponentDO> componentInfo = InsertComponentFileInfo(MasterDAL, numComponents);

            //start up the progress bar
            int totalSteps = numComponents + 3;
            ...
            {
                 String compPath = string.Format("{0}\\{1}",                     //generate the path of the comp file
                GetSaveDir(MasterDAL.Path), comp.FileName);

                 if (!File.Exists(compPath))
                 {
                     CreateComponent(MasterDAL, counter, comp, compPath);
                 }
        ...
        protected void CreateComponent(DAL masterDAL, int compNum, ComponentDO compInfo, String compPath)
        {
            ...
            catch (Exception)
            {
                ...
                try //may throw exception if file doesn't exist, but we don't care
                {
        ...
        private static List<ComponentDO> InsertComponentFileInfo(DAL masterDAL, int numComp)
        {
